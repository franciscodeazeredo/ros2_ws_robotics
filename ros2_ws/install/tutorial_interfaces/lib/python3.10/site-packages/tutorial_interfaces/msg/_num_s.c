...
bool tutorial_interfaces__msg__num__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // axes
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->axes.data;
  ...
  {  // buttons
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int64_t * dest = ros_message->buttons.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // numpad
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int64_t * dest = ros_message->numpad.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * tutorial_interfaces__msg__num__convert_to_py(void * raw_ros_message)
{
  ...
  {  // axes
    ...
    if (ros_message->axes.size > 0) {
      ...
      double * src = &(ros_message->axes.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->axes.size * sizeof(double));
      assert(data != NULL);
  ...
  {  // buttons
    ...
    if (ros_message->buttons.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int64_t * src = &(ros_message->buttons.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->buttons.size * sizeof(int64_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // numpad
    ...
    if (ros_message->numpad.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int64_t * src = &(ros_message->numpad.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->numpad.size * sizeof(int64_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
